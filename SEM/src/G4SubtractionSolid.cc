...
// 27.03.08 E.Bosch: work around for infinite loop in DistanceToIn
//
// --------------------------------------------------------------------
                                   ...
                                   const G4ThreeVector& v  ) const 
{
  ...
  if( Inside(p) == kInside )
  {
    ...
    if ( fPtrSolidB->Inside(p) != kOutside )   // start: out of B
    {
      ...
      {
        // EB: Added loopcounter to prevent infinite loops here
        int loopcount=0;
        do
        {
          loopcount++;
        ...
        while( (Inside(p+dist*v) == kOutside ) && (loopcount<100)) ;
        if (loopcount > 1) {
           G4cerr << "G4SubtractionSolid:  prevented infinite loop" << G4endl;
           G4cerr << "    Solid A:" << fPtrSolidA->GetName() << G4endl;
           G4cerr << "    Solid B:" << fPtrSolidB->GetName() << G4endl;
           G4cerr << "*** This is just a warning ***" << G4endl;
        }
      }
    }
    else // p outside A, start in A
    {
      ...
      else
      {
        // EB: Added loopcounter to prevent infinite loops here
        int loopcount=0;
        while( ( Inside(p+dist*v) == kOutside) && (loopcount < 100))  // pushing loop
        {
          loopcount ++;
          disTmp = fPtrSolidB->DistanceToOut(p+dist*v,v) ;
          dist += disTmp ;
