                                               ...
                                               G4double *s1, G4double *s2 )
{
  ...
  // EK: Changed
  // EK: In the following, the criterion was changed from std::fabs(b) to b*b to get the dimensions straight!
  // EK: furthermore, the prefactor was lowered from 1e-6 to 1e-14 so that you can still hit a nanowire from, say, a mm away
  G4double b2 = b*b;
  G4double radical = b2 - 4*a*c;
  if (radical < -1E-14*b2) return 0;    // No solution
  // EK: End of changes
  
  // EB: Changed 15-03-2012
  // EB: Original (for the same reasons as above) See BugTest/G4IntersectingCone.bugfix for a description
                                               ...
                                               G4double *s1, G4double *s2 )
{
  ...
  // EB: Changed
  // EB: In the following, the criterion was changed from std::fabs(b) to b*b to get the dimensions straight!
  // EB: furthermore, the prefactor was lowered from 1e-6 to 1e-14 so that you can still hit a nanowire from, say, a mm away
  G4double b2 = b*b;
  G4double radical = b2 - 4*a*c;
  if (radical < -1E-14*b2) return 0;    // No solution
